...
def build_agent_prompt(
    ...
    lines.extend([
        ...
        f"- **Every time** you complete a task, you MUST report your estimated token usage:",
        f"  `clawteam cost report {team_name} --agent {agent_name} --input-tokens <N> --output-tokens <N> --cost-cents <C> --provider <provider> --model <model>`",
        "- Estimate your tokens: a short task ~2K input + 1K output; a medium task ~8K in + 4K out; a large task ~20K in + 10K out.",
        "- Estimate cost in cents: Claude Opus ~$15/1M input, ~$75/1M output; Claude Sonnet ~$3/1M input, ~$15/1M output.",
        "- Example: medium task on Opus ≈ 0.12 + 0.30 = ~42 cents → `--cost-cents 42`",
        "- Do NOT skip this step. The team leader monitors budget through these reports.",
        "",
