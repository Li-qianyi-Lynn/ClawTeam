...
const IconGemini = () => (
  <svg viewBox="0 0 20 20" width="18" height="18" fill="currentColor">
    <path d="M10 2 Q10 10 18 10 Q10 10 10 18 Q10 10 2 10 Q10 10 10 2Z"/>
  </svg>
);
const IconKimi = () => (
  <svg viewBox="0 0 20 20" width="18" height="18" fill="currentColor" fillRule="evenodd">
    <path d="M2 10A8 8 0 1 0 18 10A8 8 0 1 0 2 10ZM8 9A6 6 0 1 0 20 9A6 6 0 1 0 8 9Z"/>
  </svg>
);
const IconClaude = () => (
...
const IconTaskBoard = () => (
  <svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <rect x="11" y="10" width="7" height="8" rx="1.5"/><rect x="2" y="13" width="7" height="5" rx="1.5"/>
  </svg>
);
const IconRelay = () => (
  <svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="4" cy="10" r="2.5"/><circle cx="16" cy="5" r="2.5"/><circle cx="16" cy="15" r="2.5"/>
    <path d="M6.5 9L13.5 6M6.5 11L13.5 14"/>
  </svg>
);
const IconGitBranch = () => (
  <svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="6" cy="4" r="2"/><circle cx="6" cy="16" r="2"/><circle cx="15" cy="7" r="2"/>
    <line x1="6" y1="6" x2="6" y2="14"/><path d="M8 4.5C10 4.5 13 5.5 13 7"/>
  </svg>
);

const clientIcons = { claude: IconClaude, codex: IconCodex, gemini: IconGemini, kimi: IconKimi, openclaw: IconOpenClaw, nanobot: IconNanobot, any: IconCLI };
...
const features = [
  { Icon: IconTaskBoard, title: "Shared task graph", body: "One board for blockers, priority, ownership, and progress across all agents." },
  { Icon: IconRelay, title: "Persistent coordination", body: "Handoffs, aliases, and notes stay visible after prompts scroll away." },
  { Icon: IconGitBranch, title: "Git-aware execution", body: "Real repos, worktrees, diffs, and mergeable state keep the swarm grounded." }
];
const templates = [
  {
    name: "docs-sprint",
    label: "Docs Sprint",
    roles: ["leader", "writer", "engineer", "reviewer"],
    desc: "Ship docs fast. A tight loop of drafting, building, and reviewing — until the page is live.",
    cmd: "clawteam team spawn-team sprint \\\n  --template docs-sprint \\\n  --goal \"Ship v2 reference docs\""
  },
  {
    name: "full-stack",
    label: "Engineering Swarm",
    roles: ["architect", "backend", "frontend", "security"],
    desc: "Design, build, and harden a feature end-to-end. Each agent owns a layer; the architect keeps them aligned.",
    cmd: "clawteam team spawn-team eng \\\n  --template full-stack \\\n  --goal \"Build the auth module\""
  },
  {
    name: "maomao-save-world",
    label: "猫猫拯救世界",
    roles: ["coordinator", "designer", "backend", "architect", "security", "copywriter"],
    desc: "A fully-staffed creative product team — personas, attitudes, and all. Ships the feature and the launch post in one run.",
    cmd: "clawteam team spawn-team myteam \\\n  --template maomao-save-world"
  }
];
const steps = [
      ...
      <main>
        <section className="hero shell" id="top">
          <div className="hero-content">
            ...
            <p className="hero-sub">ClawTeam is the coordination layer for Claude Code, Codex, Gemini, Kimi, OpenClaw, nanobot, and any terminal-native client that needs to plan, delegate, and ship together.</p>
        ...
        <section className="features shell" id="features">
          ...
          <div className="features-grid">{features.map(f=><article className="feature-card" key={f.title}><div className="feature-icon"><f.Icon /></div><h3>{f.title}</h3><p>{f.body}</p></article>)}</div>
        </section>
        <section className="templates shell" id="templates">
          <div className="section-header"><p className="section-label">Team templates</p><h2>One command, a full team</h2></div>
          <div className="templates-grid">
            {templates.map(t=>(
              <div className="template-card" key={t.name}>
                <strong className="template-name">{t.label}</strong>
                <div className="template-roles">{t.roles.map(r=><span className="role-tag" key={r}>{r}</span>)}</div>
                <p className="template-desc">{t.desc}</p>
                <pre className="template-cmd"><code>{t.cmd}</code></pre>
              </div>
            ))}
          </div>
        </section>
